...
export default function ParticleCanvas() {
  ...
  useEffect(() => {
    ...
    const maxDistance = 100;  // Reduced distance for faster drawing
    let mouse = { x: null, y: null };
    ...
    class Particle {
      constructor(x, y) {
        ...
        this.y = y;
        this.size = 3;
        this.vx = (Math.random() - 0.5) * 0.7; // slower speed for smoothness
    ...
    function animate() {
      ctx.fillStyle = "rgba(0,0,0,0.3)"; // Transparent background for smoothness
      ctx.fillRect(0, 0, canvas.width, canvas.height);
